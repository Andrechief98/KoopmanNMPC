...
for k=1:1:N
	...
	sol = knmpc({z_mpc(:,k), x_r(:,i_ref), Ad, Bd, bd, c_obst_curr, v_obst_curr});
	...
	if plant_model == "kin"
		x_mpc(3,k+1) = x_mpc(3,k) + Ts*u_mpc(1,k);
		x_mpc([1,2,4],k+1) = x_mpc([1,2,4],k) + Ts*f_kin(x_mpc(4,k), [x_mpc(3,k); u_mpc(2,k)]);
	end
...
for k=1:1:N
	...
	for i=1:1:n_obst
		plot_obst([obst.x_c(i) + obst.v_x(i)*(k-1)*Ts; obst.y_c(i) + obst.v_y(i)*(k-1)*Ts], ...
			[2*obst.r(i); 2*obst.r(i)], 1.25, [0.35 0.35 0.35])
	end
	...
	% Ego robot
	plot_obst([x_mpc(1,k); x_mpc(2,k)], [2*r_ego; 2*r_ego], 1.25, [0 0 1])
...
for i=1:1:n_obst
	for k=1:1:N-1
		plot_obst([obst.x_c(i) + obst.v_x(i)*(k-1)*Ts; obst.y_c(i) + obst.v_y(i)*(k-1)*Ts], ...
			[2*obst.r(i); 2*obst.r(i)], 1, [0.75 0.75 0.75])
	end
...
% Ego robot
for k=1:1:N-1
	plot_obst([x_mpc(1,k); x_mpc(2,k)], [2*r_ego; 2*r_ego], 1, [0.5 0.5 1])
end
plot_obst([x_mpc(1,N); x_mpc(2,N)], [2*r_ego; 2*r_ego], 1.25, [0 0 1])
...
plot(t, rad2deg(x_mpc(4,1:end-1)), 'b-')
